...
public class EmailConfig {
	...
	@Value("${sg.email.api.key}")
	private String sgAPIKey;

	@Value("${sg.single.sender.email}")
	private String singleSenderEmail;

	@Value("${sg.send.email.endpoint}")
	...
	@Bean(name = "requestReceivedSGTemplate")
	public Mail requestReceivedSGTemplate() {
		Email from = new Email(singleSenderEmail);//sgspringemail@mailinator.com
		Content content = new Content("text/plain", requestReceivedText);
		var mail = new Mail();
		mail.setFrom(from);
		mail.setSubject(requestReceivedSubject);
		mail.addContent(content);
		return mail;
	}

	@Bean(name = "requestProcessedSGTemplate")
	public Mail requestProcessedSGTemplate() {
		Email from = new Email(singleSenderEmail);//sgspringemail@mailinator.com
		Content content = new Content("text/plain", requestProcessedText);
		var mail = new Mail();
		mail.setFrom(from);
		mail.setSubject(requestProcessedSubject);
		mail.addContent(content);
		return mail;
	}

	@Bean(name = "sendEmailRequestSG")
	public Request sendEmailRequestSG() {
		Request request = new Request();
		request.setMethod(Method.POST);
		request.setEndpoint(sendEmailEndpoint);
		return request;
	}
